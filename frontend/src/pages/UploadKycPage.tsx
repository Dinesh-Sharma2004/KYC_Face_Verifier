...
import { createJob, uploadDocument, fetchJob } from "../api/client";
import { DocumentMeta } from "../types";
import { useActiveJob } from "../state/JobContext";

export const UploadKycPage: React.FC = () => {
  ...
  const { activeJobId, setActiveJobId, clearActiveJob } = useActiveJob();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!file && uploadedDoc) {
      navigate("/upload-supporting");
      return;
    }

    if (!file) {
      setError("Please select an ID document image containing a face photo.");
      return;
    }
    ...
    try {
      ...
      const res = await uploadDocument(jobId, file, "primary", "ID_PHOTO");
      setUploadedDoc({
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white flex items-center space-x-2">
                ...
                <span>Upload Primary ID Photo Document</span>
              </h2>
              <p className="text-xs text-slate-400">Select ID proof containing face photo (PAN, Passport, Aadhaar, Driver License)</p>
            </div>
          </div>
          {uploadedDoc && (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400 font-mono block">
                      {(uploadedDoc.size_bytes / 1024).toFixed(1)} KB • Primary ID Image
                    </span>
                  </div>
                </div>
                <label
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-2">
                {uploadedDoc && !file ? "Replace Primary Image (Optional)" : "Primary ID Photo Image"}
              </label>
              <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl p-6 text-center transition-all bg-slate-900/30">
                <input
                  type="file"
                  id={uploadedDoc && !file ? "file-upload-replace" : "file-upload"}
                  accept="image/*"
                  onChange={handleFileChange}
                  className="hidden"
                />
                <label
                  htmlFor={uploadedDoc && !file ? "file-upload-replace" : "file-upload"}
                  className="cursor-pointer space-y-3 block"
                >
                  {previewUrl ? (
                    <div className="space-y-2">
                      <img src={previewUrl} alt="Primary Face Preview" className="max-h-48 mx-auto rounded-lg border border-slate-700 shadow-md" />
                    ...
                    <div>
                      ...
                      <span className="text-sm font-medium text-indigo-400">Click to upload Primary ID photo</span>
                      <span className="text-xs text-slate-500 block mt-1">PNG, JPG, or JPEG face photo</span>
                    </div>
                  )}
                </label>
              </div>
            ...
            >
              {loading ? (
                ...
                <>
                  <span>
                    {uploadedDoc && !file ? "Continue with Saved Image" : "Save & Continue to Selfie / Secondary Photo"}
                  </span>
